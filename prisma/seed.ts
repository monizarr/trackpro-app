...
const connectionString = process.env.DATABASE_URL || 'postgresql://zar:iop@localhost:5432/trackpro-db?schema=public'
...
async function main() {
  ...
  const batchSku = `PROD-${today.getFullYear()}${String(today.getMonth() + 1).padStart(2, '0')}${String(today.getDate()).padStart(2, '0')}-001`
